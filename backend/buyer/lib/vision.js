// lib/vision.js
//
// Sends seller-uploaded photos to Mistral's Pixtral (vision) and asks it to draft a
...
async function detectListingFromPhotos(images) {
  ...
  // Pixtral takes images as data: URLs inside the user content array.
  ...
  const schemaNote =
    ...
    JSON.stringify(LISTING_DRAFT_SCHEMA);
  ...
  const message = await llm.chat({
    ...
    messages: [
      { role: "system", content: SYSTEM_PROMPT + schemaNote },
      {
        role: "user",
        content: [
          ...imageBlocks,
          { type: "text", text: "Draft a listing for the item shown in these photos." },
        ],
      },
    ],
  });

  const parsed = JSON.parse(llm.contentToText(message.content));
  const missing = (LISTING_DRAFT_SCHEMA.required || []).filter((k) => parsed[k] === undefined);
  if (missing.length > 0) {
    throw new Error(`vision draft missing required fields: ${missing.join(", ")}`);
  }
  return parsed;
}
