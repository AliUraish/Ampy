...
// Failed/empty results get a much shorter TTL than successful ones. A
// network blip or a momentary block is usually transient — caching it for
// the full 10 minutes would lock every search into the mock-data fallback
// for that long even after Craigslist is reachable again. 30s means a
// retry a few requests later can recover instead of being stuck on a
// stale failure.
...
const REQUEST_HEADERS = {
  ...
  "User-Agent": "Mozilla/5.0 (compatible; AmpyBot/1.0; +https://example.invalid/bot)",
 ...
 *
 * Never throws — timeouts, blocks/rate-limiting, unexpected markup, or
 * empty results all resolve to an empty `listings` array plus a logged
 * warning, so a scrape hiccup can never crash the server or a request that
 * depends on it.
 *
...
async function searchCraigslist({ query, location = "sfbay", category, maxPrice } = {}) {
  ...
  if (fetchError) {
    ...
    if (rows.length === 0) {
      error = "no recognizable result rows — markup may have changed, or there are genuinely no results";
    } else {
      listings = rows
        .toArray()
        .map((row) => {
  ...
  // Cache empty/error results too, so a burst of requests during a real
  // outage doesn't hammer Craigslist repeatedly — but only for a short TTL
  // (see ERROR_CACHE_TTL_MS above), not the full 10-minute success TTL.
  // Otherwise one transient blip would lock every search into the
  // mock-data fallback for 10 minutes even after Craigslist recovers.
