...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Negotiate a listing's price with the seller's agent, and get back the full exchange plus whether a deal " +
...
function buildSystemPrompt({ location, budget, canSend }) {
  return [
    "You are a buyer's agent on Ampy. A person tells you what they want to buy; you search every connected",
    ...
    "- Keep queries SHORT and generic: 1-3 words like 'road bike', 'office chair', 'macbook'. Marketplace",
    "  search matches listing text literally — long or specific queries ('54cm aluminum road bike shimano')",
    "  return junk or nothing. Filter for specifics yourself from the results; don't put them in the query.",
...
async function runBuyerAgent({
  ...
  // ask questions — the seller agent answers for Ampy listings, iMessage
  ...
  async function doDetails({ listingId }) {
    ...
    return {
      ...
      answered_by: canSend && route.channel === "seller_agent" ? "the seller's Ampy agent (usually replies immediately)" : route.channel === "imessage" ? "the seller directly, over iMessage (replies take minutes to hours)" : null,
