...
//
//   'seller_agent'  A Hagglr seller agent represents this listing (built
...
// -------------------------
// Set SELLER_AGENT_URL to point at the seller-agent service. Hagglr then
...
//
// Answering later instead is fine — POST back into Hagglr at any time:
//
...
function sellerAgentUrl() {
  return (process.env.SELLER_AGENT_URL || "").replace(/\/+$/, "");
}
...
// other half of the marketplace. 'hagglr' restricts it to listings actually
...
function sellerAgentScope() {
  ...
  return scope === "hagglr" ? "hagglr" : "all";
}
...
function resolveChannel(listing) {
  ...
  if (sellerAgentConfigured() && (sellerAgentScope() === "all" || listing.source === "seller")) {
    return {
      ...
      confidence: "high",
      reason: "Represented by a Hagglr seller agent, which answers and negotiates on the seller's behalf.",
    };
  }
