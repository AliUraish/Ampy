...
//
// Run this the moment you point Hagglr at a new seller agent. It exercises
...
const BASE = (process.argv[2] || process.env.SELLER_AGENT_URL || "").replace(/\/+$/, "");
const TIMEOUT_MS = Number(process.env.SELLER_AGENT_TIMEOUT_MS || 15000);

if (!BASE) {
  console.error("Usage: node tools/checkSellerAgent.js <url>   (or set SELLER_AGENT_URL)");
  process.exit(2);
}
